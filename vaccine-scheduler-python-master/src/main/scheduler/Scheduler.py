...
def create_patient(tokens):
    ...
    print(f"printing tokens {tokens}")
    if len(tokens) != 3:
...
def reserve(tokens):
    ...
    # First query: get the available caregiver
    cm = ConnectionManager()
    conn = cm.create_connection()
    ...
    # Second query: get available vaccine
    cm = ConnectionManager()
    conn = cm.create_connection()
    ...
    try:
        ...
        doses_list = []
        for row in cursor2:
            required_doses = int(row[0])
            doses_list.append(required_doses)
        # check if there's enough available doses
        if len(doses_list) == 0:
            required_doses = 0
    ...
    # Third query: get the latest appointmentID
    cm = ConnectionManager()
    conn = cm.create_connection()
    ...
    # Fourth query: delete the assigned caregiver from Availabilities Table
    ...
    print("Appointment ID:{}\nCaregiver username:{}".format(int(apptID), assigned_caregiver))

    # Fifth query: insert the info of the appointment into Appointments Table
    cm = ConnectionManager()
    conn = cm.create_connection()
...
def cancel(tokens):
    ...
    if current_caregiver is not None and current_patient is None:
        ...
        # First query: check this apptID's existence
        cm = ConnectionManager()
        conn = cm.create_connection()
        ...
        try:
            ...
            if len(appt_list) == 0:
                print("There's no such AppointmentID or this username does not have permission to cancel this appointment!")
        ...
        # Second query: save caregiver_name, vaccine_name, and time
        cm = ConnectionManager()
        conn = cm.create_connection()
        ...
        # Third query: delete the Appointment
        cm = ConnectionManager()
        conn = cm.create_connection()
        ...
        # Forth query: add caregiver back
        cm = ConnectionManager()
        conn = cm.create_connection()
        ...
        # Fifth query: add doses back, and need to get current doses
        cm = ConnectionManager()
        conn = cm.create_connection()
    ...
    # situation:patient login
        ...
        # Second query: safe caregiver_name, vaccine_name, and time
        cm = ConnectionManager()
        conn = cm.create_connection()
...
def show_appointments(tokens):
    ...
    if current_caregiver is not None:
        ...
        try:
            ...
            print("appointmentID", "", "vaccine name", "", "date","", "patient name")
            for row in cursor1:
        ...
        try:
            ...
            print("appointmentID", "", "vaccine name", "", "date", "", "caregiver name")
            for row in cursor2:
                print(row[0], "",row[1], "",row[2], "",row[3])
